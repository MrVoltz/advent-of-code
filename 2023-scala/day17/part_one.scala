  ...
  def bfsStep(q: Queue[Entry], vis: Map[Key, Entry]): Map[Key, Entry] = q.dequeueOption match {
    case Some((e@Entry(cur, prev, straight, loss), q2)) =>
      if(vis.get(e.key).forall(e <= _)) {
        ...
        ).filter(p => grid.isValid(p._1)).map({
          case (newCur, false) => Entry(newCur, cur, 1, loss + (grid(newCur) - '0'))
          case (newCur, true) => Entry(newCur, cur, straight + 1, loss + (grid(newCur) - '0'))
        }).filter(e => e.valid && vis.get(e.key).forall(e < _))
