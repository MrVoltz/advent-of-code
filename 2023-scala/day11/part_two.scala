...
@main def partOne(inputPath: String): Unit = {
  ...
  case class Pos(y: Long, x: Long) extends Ordered[Pos] {
    import scala.math.Ordered.orderingToOrdered

    def dist(that: Pos): Long = (y - that.y).abs + (x - that.x).abs
  ...
  @tailrec
  def parseField(y: Int, x: Int, seenEmptyRows: Long, seenEmptyCols: Long, galaxies: Set[Pos]): Set[Pos] =
    if(y >= height) galaxies
    else if(x >= width) parseField(y + 1, 0, seenEmptyRows, 0, galaxies)
    else if(emptyRows.contains(y)) parseField(y + 1, 0, seenEmptyRows + 1000000 - 1, 0, galaxies)
    else if(emptyCols.contains(x)) parseField(y, x + 1, seenEmptyRows, seenEmptyCols + 1000000 - 1, galaxies)
